...
int main()
{
    ...
    Number_t  extI_type;        // Ext. inp. type: 0-const., 1-gradual inc./dec.
    Current_t Imin;             // Minimum external input current (per sec)
    Current_t Imax;             // Maximum external input current (per sec)
    Number_t  nr_steps;         // Number of steps from minimum to maximum
    ...
    read_params(en, in, network_type, t_end, dt, params,
                ...
                tref, warm_up_time, nr_pops, sn, extI_type, Imin, Imax, nr_steps,
                fS, fR, SE, SI);
    ...
    Current_t extI_vector[n-1]; // External input vector at t
    for (int i=0; i<n; i++) { extI_vector[i]=0; }
    ...
    for(int i = 0; i < nr_time_steps; i++)
    {
        ...
        get_external_input_current(en, in, nr_time_steps, i, t_unit, extI_type,
                                   Imin, Imax, nr_steps, extI_vector);
        ...
        evolve_potential(t_current, dt, get0, git0, get1, git1, v0, en, in, A, eneff,
                         ineff, eori, lft, single_track_id, extI_vector, vE, vI, vL, gL, vThresh,
    ...
    calculate_population_statistics(n, en, in, nr_rps, t_unit, t_end,
                                    ...
                                    network_type, params, tref, SE, SI, dt, fS,
